...
async function initialize(opts) {
    ...
    if (!!config.experimental.appDir) {
        renderWorkers.app = await (0, _serveripc.createWorker)(ipcPort, ipcValidationKey, opts.isNodeDebugging, "app", config);
    }
    renderWorkers.pages = await (0, _serveripc.createWorker)(ipcPort, ipcValidationKey, opts.isNodeDebugging, "pages", config);
    // pre-initialize workers
    await ((_renderWorkers_app = renderWorkers.app) == null ? void 0 : _renderWorkers_app.initialize(renderWorkerOpts));
    await ((_renderWorkers_pages = renderWorkers.pages) == null ? void 0 : _renderWorkers_pages.initialize(renderWorkerOpts));
    if (devInstance) {
    ...
    const requestHandler = async (req, res)=>{
        ...
        async function invokeRender(parsedUrl, type, handleIndex, invokePath, additionalInvokeHeaders = {}) {
            ...
            const workerResult = await (curWorker == null ? void 0 : curWorker.initialize(renderWorkerOpts));
            if (!workerResult) {
    ...
    const { port , hostname  } = await (0, _setupserverworker.initializeServerWorker)(requestHandler, upgradeHandler, opts);
    initializeResult = {
        port,
