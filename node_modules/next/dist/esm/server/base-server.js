...
export default class Server {
    ...
    async normalizeNextData(_req, _res, _parsedUrl) {
        return {
            finished: false
        };
    }
        ...
        try {
            ...
            if (useMatchedPathHeader) {
                try {
                    ...
                    const normalizeResult = await this.normalizeNextData(req, res, parsedUrl);
            ...
            const useInvokePath = !useMatchedPathHeader && process.env.NEXT_RUNTIME !== "edge" && process.env.__NEXT_PRIVATE_RENDER_WORKER && invokePath;
            if (useInvokePath) {
            ...
            if (process.env.NEXT_RUNTIME !== "edge" && process.env.__NEXT_PRIVATE_RENDER_WORKER && req.headers["x-middleware-invoke"]) {
                const nextDataResult = await this.normalizeNextData(req, res, parsedUrl);
                if (nextDataResult.finished) {
                    return;
                }
   ...
   */ async prepare() {
    ...
    async renderToResponseWithComponentsImpl({ req , res , pathname , renderOpts: opts  }, { components , query  }) {
        ...
        const doRender = async ()=>{
            ...
            let result;
            if (((_components_routeModule = components.routeModule) == null ? void 0 : _components_routeModule.definition.kind) === RouteKind.APP_ROUTE) {
            ...
            } else if (((_components_routeModule1 = components.routeModule) == null ? void 0 : _components_routeModule1.definition.kind) === RouteKind.PAGES) {
            ...
            } else if (((_components_routeModule2 = components.routeModule) == null ? void 0 : _components_routeModule2.definition.kind) === RouteKind.APP_PAGE) {
                const module = components.routeModule;
    ...
    async renderToResponseImpl(ctx) {
        ...
        try {
            for await (const match of this.matchers.matchAll(pathname, options)){
                ...
                if (!this.minimalMode && this.isRenderWorker && typeof invokeOutput === "string" && isDynamicRoute(invokeOutput || "") && invokeOutput !== match.definition.pathname) {
                    continue;
                }
